...
public func EvaluateAndPingBodyWalkState() {
    ...
    if IsDefined(psmBlackboard) {
        let hlState = psmBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.HighLevel);
        ...
        if hlState == EnumInt(gamedataHighLevelState.InVehicle) || vehicleState > 0 {
            stateName = "vehicle";
        } 
        else if hlState == EnumInt(gamedataHighLevelState.SceneTier3) || hlState == EnumInt(gamedataHighLevelState.SceneTier4) || hlState == EnumInt(gamedataHighLevelState.SceneTier5) {
            stateName = "scene";
        } 
        else if combatState == EnumInt(gamePSMCombat.InCombat) {
            stateName = "combat";
        }
    }
